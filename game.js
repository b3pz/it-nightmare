...
const rooms=[
{name:"EDITORIA",x:30,y:55,w:250,h:210,f:"stone"},{name:"LOFT",x:300,y:55,w:205,h:210,f:"stone"},{name:"SERVER",x:525,y:55,w:230,h:210,f:"server"},
...
{x:535,y:560,room:"CENTRALE",kind:"PC"},{x:940,y:285,room:"SALA MEET",kind:"MEETING"},{x:1135,y:285,room:"THE BUNKER",kind:"PC"},{x:1345,y:285,room:"RENDERISTI",kind:"PC"},{x:1010,y:520,room:"SPAZIO A",kind:"PC"},
...
function spawnAmbient(){
 ...
 ambientNPCs=seats.slice(0,14).map((s,i)=>({
...
function generateNpcActivityTicket(n){
 ...
 tickets.push({id:crypto.randomUUID?crypto.randomUUID():Math.random()+"",level,p,due:Math.min(BOSS-.2,state.min+mins),q:drawQuestion(categoryForStation(p)),taskType:type,source:n.name,expired:false});
...
function updateAmbient(dt){
 ...
 for(const n of ambientNPCs){
   ...
   if(n.state==="work"&&n.timer<=0){
     const r=Math.random();
     n.activity=r<.30?"coffee":r<.52?"meeting":r<.70?"printer":r<.80?"gallery":"wander";
...
function makeCarryMission(){
 ...
 if(r<.20){
   ...
   const rec=availableRecipient("CENTRALE");
   m={label:"CONSEGNA",item:"MOUSE USB",pickup:{x:185,y:640,room:"IT"},to:{x:rec.x,y:rec.y,room:"CENTRALE"},recipient:rec,targetType:"npc"};
 }
 m.stage="pickup";
 return m;
}
...
function carryTarget(){
 ...
 if(carryMission.recipient){
   // Recipient may be walking: delivery follows the person.
   return {x:carryMission.recipient.x,y:carryMission.recipient.y,room:carryMission.to.room};
 }
...
function updateTaskProgress(){
 if(!state)return;
 let pct;
 if(state.phase==="ended")pct=100;
 else if(state.phase==="boss")pct=95;
 else{
   const timeRatio=Math.max(0,Math.min(1,(state.min-START)/(BOSS-START)));
   const timePart=timeRatio*78;
   const solvedPart=Math.min(12,state.solved*.7);
   const quality=Math.max(-6,Math.min(4,(state.rep-3)*1.2-state.strikes*1.5-state.incident*.035));
   pct=Math.max(0,Math.min(94,Math.round(timePart+solvedPart+quality)));
 }
...
function newTicket(force,opts={}){
 ...
 tickets.push({id:crypto.randomUUID?crypto.randomUUID():Math.random()+"",level,p,due:Math.min(BOSS-.2,state.min+mins),q:drawQuestion(categoryForStation(p)),taskType:useMini?taskTypeForStation(p):null,criticalFrom:level==="CRITICAL"?bosses[Math.floor(Math.random()*bosses.length)]:null,source:opts.source||"USER",expired:false});
 renderTickets();
}
...
function interact(){
 ...
 if(introStage==="outside"&&Math.hypot(player.x-650,player.y-842)<72){startShiftFromEntrance();return}
...
function draw(){
 ...
 // V5.1 — ingresso esterno realmente giocabile
 if(introStage==="outside"){
   g.fillStyle="#1a1d1b";g.fillRect(520,855,185,45);
   g.strokeStyle="#49534d";g.lineWidth=3;g.strokeRect(520,855,185,45);
   ...
   if(Math.hypot(player.x-650,player.y-842)<90){g.fillStyle="#ffd447";g.font="bold 12px monospace";g.fillText("E — ENTRA",620,850)}
 }
